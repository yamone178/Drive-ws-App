        ...
        <div class="row">
            ...
            @foreach($folders as $folder)
                <div class="col-3 position-relative">
                    ...
                    <div class="card my-3" style="height: 50px;  text-decoration: none" >
                    ...
                    <div class="dropdown position-absolute" style="top: 10px; right: 10px;">
                        <button class=" btn btn-sm  border-0 dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        </button>
                        <ul class="dropdown-menu">
                            <li>
                                <form action="{{route("folder.restore", $folder->id)}}" id="restore{{$folder->id}}" method="post">
                                    @csrf
                                    <button   class="dropdown-item" form="restore{{$folder->id}}">restore</button>
                                </form>
                            </li>

                            <li>
                                <form action="{{route("folder.destroy", $folder->id)}}" id="deleteFolder{{$folder->id}}" method="post">
                                    @csrf
                                    @method('delete')
                                    <button   class="dropdown-item" form="deleteFolder{{$folder->id}}">delete forever</button>
                                </form>
                            </li>
                        </ul>
                    </div>
                        ...
                        @endif
                            ...
                            <div class="dropdown position-absolute" style="top: 10px; right: 10px;">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li>
                                        <form action="{{route("file.restore", $file->id)}}" id="restore{{$file->id}}" method="post">
                                            @csrf
                                            <button   class="dropdown-item" form="restore{{$file->id}}">restore</button>
                                        </form>
                                    </li>

                                    <li>
                                        <form action="{{route("file.destroy", $file->id)}}" id="deleteFolder{{$file->id}}" method="post">
                                            @csrf
                                            @method('delete')
                                            <button   class="dropdown-item" form="deleteFolder{{$file->id}}">delete forever</button>
                                        </form>
                                    </li>
                                </ul>
                            </div>
